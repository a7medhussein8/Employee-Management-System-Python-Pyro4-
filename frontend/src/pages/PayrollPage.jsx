...
export default function PayrollPage() {
  ...
  // ✅ Calculate payroll (matches backend /payroll/calculate)
  const calcPayroll = async () => {
    if (!empId || !base) {
      setMessage("⚠️ Please enter Employee ID and Base Salary!");
      return;
    }

    try {
      setLoading(true);
      const res = await axios.post("http://localhost:5000/payroll/calculate", {
  ...
  const loadHistory = async () => {
    if (!empId) {
      setMessage("⚠️ Enter Employee ID first to view history!");
      return;
    }

    try {
      setLoading(true);
      const res = await axios.get(`http://localhost:5000/payroll/history/${empId}`);
      setHistory(res.data || []);
  ...
  return (
    <div className="page">
      ...
      <div className="card">
        ...
        {history.length > 0 ? (
          <TableView
            headers={["Date", "Month", "Base", "Bonus", "Deductions", "Total"]}
            rows={history.map((h) => ({
              date: h.date || "-",
              month: h.month || "-",
              base: `$${h.base?.toFixed?.(2) || 0}`,
              bonus: `$${h.bonus?.toFixed?.(2) || 0}`,
              deductions: `$${h.deductions?.toFixed?.(2) || 0}`,
